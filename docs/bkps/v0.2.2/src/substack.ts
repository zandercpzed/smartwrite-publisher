...
export class SubstackService {
	...
	private async getPublicationId(): Promise<number | null> {
		...
		const strategies = [
			...
			// 4. Fallback: Parsear HTML da página inicial
			{
				name: 'HTML Homepage',
				url: `${this.baseUrl}/`
			}
			...
			try {
				...
				if (response.status === 200) {
					if (isHtmlStrategy) {
						...
						// Procura por diversos padrões de ID no HTML/JS
						const patterns = [
							/"publication_id":\s*(\d+)/,
							/publication_id\s*[:=]\s*(\d+)/,
							/["']id["']\s*:\s*(\d+)/, // Pode ser genérico, mas no contexto de meta tags ajuda
	...
	isConfigured(): boolean {
		return this.cookie.length > 0 && this.hostname.length > 0;
	}
}
