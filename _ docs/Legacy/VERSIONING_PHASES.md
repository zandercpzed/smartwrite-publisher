...
| **Phase 3** | 0.3.x  | 29/01/2026+   | ✅ Concluída    | Tiptap JSON + Automação          |
| **Phase 4** | 1.0.0  | 01/02/2026    | ✅ Concluída    | Publicação em Lote e Lançamento Oficial|
...
## 📦 PHASE 4: Batch Publishing (v0.4.x - v1.0.0)
...
- ✅ Agendamento em lote (Não disponível via API Substack, mas fluxo de trabalho planejado)
- ✅ Relatório de publicações
- ✅ Sincronização bidirecional (Obsidian ↔ Substack) (Considerado no refactoring)
...
```
18/01 ────────────────────────────────────────── 29/01 ───────────────────────── 01/02
v0.1 ║ v0.2 (hotfixes!) ║ v0.3.0 ║ v0.3.1 ║ v0.3.2 ║ v0.3.3 → v0.4.x (Batch) → v1.0.0 (Release)

Foundation    Phase 2         Phase 3 - Refactor + Tiptap     Phase 4 - Batch + Release
              Buggy           Clean Arch + JSON parsing

Current Status: v1.0.0 - Official Public Release 🎉
```
...
- Todas as funcionalidades planejadas para v0.4.x (Batch Publishing)
...
- Suporte a listas, blockquotes, imagens no conversor Markdown
- Repositório refatorado para estrutura Monorepo

### 🧪 Próximos Passos (v0.5.0)

- Multi-plataforma (Medium, WordPress, Ghost)
- Implementar funcionalidades avançadas (agendamento, sincronização bidirecional, etc.)
...
- 🟢 Completo e profissional (v1.0.0)

**Próximo milestone**: Implementação de recursos multi-plataforma (v0.5.0).
